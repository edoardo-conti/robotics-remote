...
* **Studente: Edoardo Conti**
* **Matricola: 278717**
* **Email: e.conti10@campus.uniurb.it**
...
Per dotare il Single-Board Computer(SBC) Arduino di storage permanente si è installato un lettore di schede microSD il quale ha richiesto, data la scheda in uso, l'impiego dell'header ICSP. Grazie a questa aggiunta è possibile salvare in modo permanente e consistente parametri di configurazione, tra i quali ssid e password della rete WiFi al quale l'Arduino deve connettersi all'avvio.
...
| ~~Driver MOSFET Ventola~~ | 2x servomotori tramite Driver DRV8838 (`VM`)  | - | 2x Sharp GP2Y0A51SK0F |
...
- **SharpDistSensor.h** : Elaborazione dei segnali in ingresso dei sensori Sharp IR. La libreria si dedica all'analisi dei segnali in ingresso restituendo un valore in unità millimetriche della distanza letta calcolata tramite filtro mediano con finestra di 3 campioni per migliorare l'attendibilità dei risultati.
...
1. **Controllo Remoto** : controllo movimenti robot tramite App con algoritmo di self-protection attivo. Il robot rimane in ascolto di richieste HTTP per controllare gli attuatori. Alla ricezione di una richiesta, questa verrà verificata e successivamente eseguita. I movimenti disponibili sono dei classici comandi *WASD*: avanti, indietro, ruotare a sinistra o destra e fermarsi. Il comando può essere seguito da un periodo di delay il quale mantiene il movimento per un determinato periodo (in millisecondi) oppure instantaneo, ergo immeditamente risolto. In questa modalità è attivo l'algoritmo di self-protection il quale si occupa di rimodulare il percorso del robot in caso di rilevazione di ostacoli.
2. **Blockly** : approcco alla programmazione a blocchi in totale sicurezza tramite sezione dell'App dedicata. In modo similare a quanto accade per il Controllo Remoto, il web server attende richieste HTTP dal codice generato dal workspace Blockly. Anche in questa modalità è attivo in background l'algoritmo di prevenzione delle collisioni per permettere una programmazione del robot in sicurezza.
...
Durante il movimento del robot tali livelli vengono constantemente aggiornati. Ogni lettura dei sensori va ad aggiornare un array cosiffatto: `short levels[4] = {3, 3, 3, 3}`, il quale definisce il "Livello" per ogni lato del robot: `{front_right, front_left, side_right, side_left}`. Quest'informazione sarà utile alla parte decisionale dell'algoritmo nel ri-modulare il proprio andamento. Infatti il percorso del robot varierà sulla base dell'analisi dei Livelli, un pò come l'approccio di un veicolo di Braitenberg. Un veicolo di Braitenberg è generalmente definito come un agente il quale può muoversi autonomamente in un'area basandosi unicamente sugli input dei sensori di cui è dotato. Le ruote (coadiuvate dai motori) fungono da attuatori e nella configurazione più semplice adottabile queste sono direttamente collegate all'uscita dei sensori, in modo che il segnale grezzo di quest'ultimi produca un effetto immediato sul movimento della ruota interessata. Quindi, ritornando all'algoritmo di self-protection, in ogni istante le decisioni prese dipenderanno principalmente dal Livello dei sensori:
```
raccolta dati da letture sensori di prossimità
analisi dati e popolamento array _levels[]

se almeno 1 elemento in _levels[] è <= 2 :
    imposta flag lvMedio come ATTIVO
altrimenti se almeno 1 elemento in levels[] è <= 1 :
    imposta flag lvBasso come ATTIVO

se lvMedio è ATTIVO :
    imposta velocità a 80%
    
se lvBasso è ATTIVO :
    evita ostacolo
    finché direzione corrente != LIBERA
        ruota in direzione candidata con maggiore successo 
    riprendi ultimo comando noto
```
Quindi se in un istante si rileva un Livello 2, l'algoritmo si limita a ridurre la velocità di movimento dei servomotori. Se presente anche un Livello 1 l'algoritmo evita l'ostacolo ruotando il robot con angoli incrementali con funzione dipendente dalla distanza dall'oggetto e riprende l'ultimo movimento noto nella direzione libera individuat, riprendendo la velocità di default.

### Web Server ###
La gestione delle richieste HTTP è affidata al web server instaurato sfruttando la liberia **WiFiNINA**. I metodi a disposizione sono di basso livello pertanto non è possibile un approccio simile ad altri linguaggi o tecnologie che più si prestano all'instaurazione di web service quali *nodejs*, *golang* e altri. Infatti è (quasi) obbligata la scelta di leggere byte per byte la richiesta in entrata (`char c = client.read()`). Questa grande limitazione motiva la scelta di sfruttare la libreria **Regexp** per filtrare le varie richieste, analizzando le linee lette tramite espressioni regolari. Di seguito una tabella che riassume tutte le richieste HTTP che il web server Arduino prende in gestione:
...
`POST /api/ping?auth=<authcode>` | richiesta per verificare la connessione tra i due agenti
...
Per realizzare l'App dedicata all'interazione con il robot la scelta sulla tecnologia da impiegare è ricaduta su **React Native (RN)**, un potente e moderno framework mantenuto da Facebook, assieme ad una ricca community, per scrivere applicazioni native utilizzando React.

Come piattaforma di sviluppo si è sfruttato **Expo**, un set di strumenti e servizi costruiti e pensati attorno a React Native e piattaforme native che semplificano lo sviluppo, compilazione e distribuzione di app iOS, Android e Web App a partire dalla stessa sorgente di codice JavaScript/TypeScript.
...
L'applicazione è stata sviluppata per essere cross-platform e quindi disponibile sia per iOS che Android. I requisiti minimi richiedono una versione di Android 5+ e di iOS 10+ . Expo prevede due diversi approcci per creare applicazioni: workflow Managed o workflow Bare.
...
La Graphical User Interface (GUI) vanta di una toolbar dal quale è possibile scegliere e posizione all'interno del workspace blocchi funzionali divisi per categorie d'appartenenza. Al momento sono disponibili blocchi delle seguenti categorie: Logic, Loops, Math, Text, *Robot*(custom) e Variables.
